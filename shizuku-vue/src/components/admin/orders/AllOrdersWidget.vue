...
import { getAllOrdersForAdminAPI, getAdminOrderDetailAPI, updateOrderStatusAPI, cancelOrderForAdminAPI } from '@/api/adminOrder'
...
<template>
  <div class="p-6 max-w-7xl mx-auto">
    <div class="flex justify-between items-center mb-6">
      ...
      <button @click="fetchOrders" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded shadow transition-colors duration-200">
        重新整理
      </button>
    </div>
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border mb-6 flex flex-wrap gap-4 items-end">
      <div class="flex-1 min-w-[200px]">
        ...
        <input v-model="searchQuery" type="text" placeholder="輸入 ORD..." class="w-full border-gray-300 border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
      </div>
      <div class="w-48">
        <label class="block text-sm font-medium text-gray-700 mb-1">訂單狀態</label>
        <select v-model="statusFilter" class="w-full border-gray-300 border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
    ...
    <div class="bg-white rounded-lg shadow-sm border overflow-x-auto">
      <table class="min-w-full text-left border-collapse">
        ...
        <tbody class="text-sm divide-y divide-gray-100">
          ...
          <tr v-else v-for="order in filteredOrders" :key="order.orderNo" class="hover:bg-gray-50 transition-colors">
            ...
            <td class="py-3 px-4 text-right font-medium text-gray-900">NT$ {{ order.totalAmount?.toLocaleString() }}</td>
            ...
            <td class="py-3 px-4 text-center">
              <button @click="openDetailModal(order.orderNo)" class="text-blue-600 hover:text-blue-800 font-medium hover:underline">查看 / 編輯</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <!-- 訂單明細與編輯 Modal -->
    <div v-if="isModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 transition-opacity">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
        <!-- Modal Header -->
        <div class="px-6 py-4 bg-gray-50 border-b flex justify-between items-center">
          <h2 class="text-xl font-bold text-gray-800">訂單明細 <span class="font-mono text-lg text-blue-600 ml-2">{{ selectedOrder?.orderNo }}</span></h2>
          ...
          <div v-if="selectedOrder" class="space-y-6">
            ...
            <div class="bg-white p-4 rounded-lg border shadow-sm">
              ...
              <div class="grid grid-cols-2 gap-4 text-sm">
                <p><span class="text-gray-500 w-16 inline-block">姓名：</span> <span class="font-medium text-gray-800">{{ selectedOrder.receiverName }}</span></p>
                <p><span class="text-gray-500 w-16 inline-block">電話：</span> <span class="font-medium text-gray-800">{{ selectedOrder.receiverPhone }}</span></p>
                <p class="col-span-2"><span class="text-gray-500 w-16 inline-block">地址：</span> <span class="font-medium text-gray-800">{{ selectedOrder.receiverAddress }}</span></p>
                <p class="col-span-2"><span class="text-gray-500 w-16 inline-block">備註：</span> <span class="font-medium text-gray-800">{{ selectedOrder.note || '無' }}</span></p>
              </div>
            </div>
            ...
            <div class="bg-blue-50 p-5 rounded-lg border border-blue-200 mt-6 shadow-sm">
              ...
              <div class="flex items-center gap-4">
                <select v-model="newStatus" class="border border-blue-300 bg-white rounded-md px-4 py-2 w-64 focus:ring-2 focus:ring-blue-400 focus:outline-none font-medium">
                  <option :value="1">待付款</option>
                  <option :value="2">已付款</option>
                  <option :value="3">已出貨</option>
                  <option :value="4">已完成</option>
                  <option :value="5">已取消 (強制退款與回補庫存)</option>
                </select>
                <button @click="saveStatus" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-md font-medium shadow transition-colors">
                  確定儲存
                </button>
              </div>
              <p v-if="newStatus == 5" class="text-sm text-red-600 mt-3 font-bold bg-red-50 p-2 rounded inline-block border border-red-200">
                ⚠️ 注意：選擇取消將會觸發「庫存回補」，請謹慎操作。
              </p>
            </div>

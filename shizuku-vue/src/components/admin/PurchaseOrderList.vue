...
<template>
  <div class="rounded-lg border border-slate-200 bg-white shadow-sm">
    <div class="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 md:flex-row md:items-center">
      <div class="min-w-0">
      ...
      <div class="relative w-full md:ml-4 md:max-w-sm">
        ...
        <input v-model="keyword" type="text" placeholder="搜尋單號、廠商、異動類型"
      ...
      <button @click="router.push({ name: 'admin-inventory-create' })"
      ...
      <table class="w-full min-w-[1040px] text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <tr v-for="order in pagedOrders" :key="order.fId" class="transition hover:bg-slate-50">
            ...
            <td class="px-5 py-4">
              <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-medium" :class="getTypeClass(order.fType)">
                {{ order.fType }}
              </span>
            </td>
            ...
            <td class="px-5 py-4">
              <div class="flex justify-end gap-1">
                <button @click="viewOrder(order.fId)"
  ...
  <!-- 詳細 Modal -->
  <div v-if="showDetailModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50"
    @click.self="showDetailModal = false">
    <div class="bg-white rounded-xl w-full mx-4 flex flex-col" style="max-width: 900px; max-height: 90vh">
      <!-- Header -->
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
        <div class="flex items-center gap-3">
          ...
          <span class="px-2 py-0.5 rounded-full text-xs font-medium" :class="getTypeClass(currentOrder?.fType)">
            {{ currentOrder?.fType }}
          </span>
      ...
      <!-- 進貨單資訊 -->
      <div class="grid grid-cols-4 gap-4 px-6 py-3 bg-gray-50 border-b border-gray-100 shrink-0 text-xs">
        ...
        <table class="w-full text-xs">
          <thead>
            <tr class="bg-gray-50">
              ...
              <th class="px-3 py-2 text-left text-gray-500 font-medium border-b border-gray-100 w-24">
                成本價
              </th>
              <th class="px-3 py-2 text-left text-gray-500 font-medium border-b border-gray-100 w-24">
                小計
          ...
          <tbody>
            <tr v-for="detail in currentOrder?.fDetails" :key="detail.fId"
          ...
          <div class="text-right space-y-1 text-xs text-gray-500">
            <template v-if="!noSupplierTypes.includes(currentOrder?.fType)">
              <div class="flex justify-between gap-16">
                <span>未稅金額</span>
                <span class="text-gray-700">NT${{ currentOrder?.fUntaxedAmount?.toLocaleString() }}</span>
              </div>
              <div class="flex justify-between gap-16">
              ...
              <div class="flex justify-between gap-16 border-t border-gray-200 pt-1">
                ...
                <strong class="text-indigo-600 text-sm">NT${{ currentOrder?.fTotalAmount?.toLocaleString() }}</strong>
              </div>
            </template>
            <template v-else>
              總數量：<strong class="text-gray-700">{{ currentOrder?.fTotalQuantity }} 件</strong>
  ...
  <div v-if="showEditModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50"
    ...
    <div class="bg-white rounded-xl w-full mx-4 max-w-sm">
      ...
      <div class="px-6 py-4 space-y-3">
        ...
        <p class="text-xs text-emerald-500 font-medium" v-if="editingStatus === '已完成'">
          ✓ 改為已完成後，庫存將自動更新
        </p>
      </div>
      <!-- Footer -->

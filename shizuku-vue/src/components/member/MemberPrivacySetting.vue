...
const sessions = ref([
    { device: 'Windows 11 · Chrome', location: 'Kaohsiung, TW', current: true },
...
<template>
    <div class="max-w-2xl mx-auto py-8">
                ...
                <div v-for="session in sessions" :key="session.device" 
                    ...
                    <div>
                        <p class="font-medium text-slate-700">{{ session.device }} 
                            <span v-if="session.current" class="ml-2 text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded">目前裝置</span>
                        </p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
            ...
            <div class="flex items-center justify-between">
                ...
                <button @click="is2FAEnabled = !is2FAEnabled" 
                    ...
                    <span :class="['absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform duration-300', is2FAEnabled ? 'translate-x-6' : '']"></span>
                </button>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-red-100 p-6">
            ...
            <button class="w-full py-2.5 border border-red-500 text-red-500 rounded-lg hover:bg-red-50 transition-colors font-medium">
                申請永久刪除帳號
            </button>
        </div>
    </div>

    ...
    <div class="relative min-h-screen flex items-center justify-center p-6">
        ...
        <div
            ...
            <div class="text-center mb-10">
                ...
                <p class="text-slate-600 mt-2 text-base">請跟隨步驟驗證您的身份並建立新密碼。</p>
            </div>
            ...
            <div v-if="step === 1" class="space-y-6">
                <div>
                    ...
                    <input type="email" v-model="email" placeholder="example@shizuku.com" :disabled="isLoading"
                        class="w-full px-4 py-3.5 bg-white/60 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/50 outline-none transition placeholder:text-slate-400 text-slate-900 disabled:bg-gray-100 disabled:text-gray-400">
                </div>

                <button @click="handleSendEmail" :disabled="isLoading"
                    class="w-full py-4 bg-blue-700 text-white rounded-xl font-bold hover:bg-blue-900 transition-all shadow-lg active:scale-95 text-base disabled:bg-blue-300 disabled:cursor-not-allowed flex items-center justify-center">
            ...
            <div v-if="step === 2" class="space-y-6">
                <div class="text-center">
                    <p class="text-sm text-slate-500 mb-1">您的驗證碼已透過電子郵件傳送至</p>
                    <p class="text-sm text-slate-800 font-semibold mb-4">{{ email }}</p>
                </div>

                <div>
                    <input type="text" v-model="code" placeholder="請輸入 6 位數驗證碼" maxlength="6" :disabled="isLoading"
                        class="w-full h-14 border-b-2 border-slate-300 text-2xl text-center tracking-[8px] focus:border-blue-600 focus:outline-none bg-transparent box-border disabled:text-slate-400 text-slate-900" />
                </div>

                <div class="text-xs text-center text-slate-400">
                    <span v-if="isCounting">{{ countdown }} 秒後重新傳送</span>
                    <span v-else class="underline"
                        :class="isLoading ? 'text-slate-400 cursor-not-allowed' : 'text-blue-600 cursor-pointer'"
                ...
                <div class="space-y-4">
                    <div class="relative">
                        <input :type="isPasswordVisible ? 'text' : 'password'" v-model="newPassword"
                            ...
                            class="w-full px-4 py-3.5 pr-10 bg-white/60 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/50 outline-none transition placeholder:text-slate-400 text-slate-900 disabled:bg-gray-100 disabled:text-gray-400" />
                        <i @click="isPasswordVisible = !isPasswordVisible"
                            :class="['pi cursor-pointer absolute right-3 top-4 text-gray-400', isPasswordVisible ? 'pi-eye-slash' : 'pi-eye']"></i>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <i @click="isConfirmVisible = !isConfirmVisible"
                            :class="['pi cursor-pointer absolute right-3 top-4 text-gray-400', isConfirmVisible ? 'pi-eye-slash' : 'pi-eye']"></i>
                    </div>

                    <p v-if="confirmPassword && newPassword !== confirmPassword" class="text-red-500 text-xs pl-1">
                        兩次輸入的密碼不一致</p>

                    <ul class="text-sm space-y-1 bg-slate-50/50 p-4 rounded-xl border border-slate-100">
            ...
            <!-- 後端回傳錯誤訊息 -->
            <p v-if="errorMessage" class="text-red-500 text-xs text-center mt-4 font-semibold">{{ errorMessage }}</p>

...
<template>
  ...
  >
    ...
    <div
      class="absolute top-[-60px] left-[-60px] w-72 h-72 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none"
    ></div>
    ...
    <!-- 選單區 -->
    <nav
      class="relative z-10 flex-1 px-3 py-6 flex flex-col gap-1.5 overflow-y-auto custom-scrollbar"
    >
      ...
      <template v-for="item in menuItems" :key="item.label">
        ...
        <div v-if="item.children">
          ...
          >
            ...
            <i
              class="pi pi-chevron-down ml-auto text-xs transition-transform duration-300"
              :class="productMenuOpen ? 'rotate-180' : ''"
            ></i>
          </button>

          <div v-show="productMenuOpen" class="ml-4 mt-1 flex flex-col gap-1">
            <router-link
            ...
            >
              ...
              >
                ...
                <div
                  v-if="isExactActive"
                  class="ml-auto w-1.5 h-1.5 rounded-full bg-indigo-300 shadow-[0_0_8px_rgba(165,180,252,0.8)]"
                ></div>
              </button>
            </router-link>
          </div>
    ...
    <div class="relative z-10 px-3 py-6">
      ...
      >
        <i
          class="pi pi-sign-out text-base w-5 text-center group-hover:-translate-x-1 transition-transform"
        ></i>

...
<nav class="relative z-10 flex-1 px-3 py-6 flex flex-col gap-1.5 overflow-y-auto custom-scrollbar">
  ...
  <template v-for="item in menuItems" :key="item.label">
    ...
    <div v-if="item.children">
      <button @click="productMenuOpen = !productMenuOpen" :class="[
        ...
        <i class="pi pi-chevron-down ml-auto text-xs transition-transform duration-300"
           :class="productMenuOpen ? 'rotate-180' : ''"></i>
      </button>

      <div v-show="productMenuOpen" class="ml-4 mt-1 flex flex-col gap-1">
        <router-link v-for="child in item.children" :key="child.label"
                     :to="child.to" custom v-slot="{ isExactActive, navigate }">

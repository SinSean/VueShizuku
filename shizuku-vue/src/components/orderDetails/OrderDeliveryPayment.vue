...
<template>
  <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex-1 h-full">
    <h2 class="text-lg font-bold text-gray-800 border-l-4 border-indigo-400 pl-3 mb-6 flex items-center gap-2">
      <i class="pi pi-truck text-indigo-400"></i> 配送與付款
    </h2>
    <div class="flex flex-col gap-5 text-gray-700">
      ...
      <div class="flex items-start gap-4 p-4 bg-gray-50/50 rounded-xl border border-gray-50">
        ...
        <div class="flex flex-col">
          <span class="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1">收件地址</span>
          <span class="text-gray-700 leading-relaxed">{{ props.order.receiverAddress }}</span>
        </div>
      </div>

      <div class="flex items-center justify-between p-4 bg-indigo-50/30 rounded-xl border border-indigo-50 mt-2">
